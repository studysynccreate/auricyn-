...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<auricynAction type="shell">${commands.setupCommand}</auricynAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<auricynAction type="start">${commands.startCommand}</auricynAction>
`;
  }
...
<auricynArtifact id="project-setup" title="Project Setup">
${commandString}
</auricynArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeAuricynArtifactTags(input: string) {
  // Regular expression to match auricynArtifact tags and their content
  const regex = /(<auricynArtifact[^>]*>)([\s\S]*?)(<\/auricynArtifact>)/g;
...
export function escapeAuricynAActionTags(input: string) {
  // Regular expression to match auricynArtifact tags and their content
  const regex = /(<auricynAction[^>]*>)([\s\S]*?)(<\/auricynAction>)/g;
...
export function escapeAuricynTags(input: string) {
  return escapeAuricynArtifactTags(escapeAuricynAActionTags(input));
}
