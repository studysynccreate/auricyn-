  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__auricynThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__auricynArtifact__', '__auricynThought__', '__auricynQuickAction', '__auricynSelectedElement__'],

      // ['className', '__auricynThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__auricynArtifact__', '__auricynThought__', '__auricynQuickAction'],
    ],
  },
  strip: [],
};
