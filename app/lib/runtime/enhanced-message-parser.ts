...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<auricynArtifact') || input.includes('</auricynArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<auricynArtifact id="${artifactId}" title="${title}" type="bundled">
<auricynAction type="file" filePath="${filePath}">
${content}
</auricynAction>
</auricynArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<auricynArtifact id="${artifactId}" title="Shell Command" type="shell">
