...
export function simplifyAuricynActions(input: string): string {
  // Using regex to match auricynAction tags that have type="file"
  const regex = /(<auricynAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/auricynAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<auricynAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</auricynAction>`;
    });

  return `<auricynArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</auricynArtifact>`;
}
