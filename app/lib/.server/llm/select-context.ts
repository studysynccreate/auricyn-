...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyAuricynActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyAuricynActions(content);

      content = content.replace(/<div class=\\"__auricynThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Auricyn failed to select files`);
  }
