...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          auricyn_user_profile: this._safeGetItem('auricyn_user_profile'),
          auricyn_settings: this._safeGetItem('auricyn_settings'),
          auricyn_profile: this._safeGetItem('auricyn_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('auricyn_viewed_features'),
          developer_mode: this._safeGetItem('auricyn_developer_mode'),
        ...
        ui: {
          // Tab configuration
          auricyn_tab_configuration: this._safeGetItem('auricyn_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('auricyn_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('auricyn_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          auricyn_read_logs: this._safeGetItem('auricyn_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('auricyn_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('auricyn_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.auricyn_tab_configuration) {
        try {
          this._safeSetItem('auricyn_tab_configuration', data.ui.auricyn_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'auricyn_acknowledged_debug_issues',
        'auricyn_acknowledged_connection_issue',
        'error_logs',
        'auricyn_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('auricyn_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
