...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      auricynVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.auricynVersion) {
      body += `- auricyn: ${data.environmentInfo.auricynVersion}\n`;
    }
  ...
  body += '---\n*Submitted via auricyn bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/auricyn';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'auricyn-bug-reporter',
    });
