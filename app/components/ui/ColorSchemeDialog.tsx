  ...
  const renderColorSection = () => (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-auricyn-elements-textPrimary flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-auricyn-elements-item-contentAccent"></div>
          Color Palette
        </h3>
        <button
          onClick={handleReset}
          className="text-sm bg-transparent hover:bg-auricyn-elements-bg-depth-2 text-auricyn-elements-textSecondary hover:text-auricyn-elements-textPrimary rounded-lg flex items-center gap-2 transition-all duration-200"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {paletteRoles.map((role) => (
          <div
            key={role.key}
            className="group flex items-center gap-4 p-4 rounded-xl bg-auricyn-elements-bg-depth-3 hover:bg-auricyn-elements-bg-depth-2 border border-transparent hover:border-auricyn-elements-borderColor transition-all duration-200"
          >
            <div className="relative flex-shrink-0">
              <div
                className="w-12 h-12 rounded-xl shadow-md cursor-pointer transition-all duration-200 hover:scale-110 ring-2 ring-transparent hover:ring-auricyn-elements-borderColorActive"
              ...
              <input
                ...
                tabIndex={-1}
              />
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-auricyn-elements-bg-depth-1 rounded-full flex items-center justify-center shadow-sm">
                <span className="i-ph:pencil-simple text-xs text-auricyn-elements-textSecondary" />
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-auricyn-elements-textPrimary transition-colors">{role.label}</div>
              <div className="text-sm text-auricyn-elements-textSecondary line-clamp-2 leading-relaxed">
                {role.description}
              </div>
              <div className="text-xs text-auricyn-elements-textTertiary font-mono mt-1 px-2 py-1 bg-auricyn-elements-bg-depth-1 rounded-md inline-block">
                {palette[role.key]}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {designFonts.map((f) => (
          <button
            ...
            className={`group p-4 rounded-xl border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-auricyn-elements-borderColorActive ${
              font.includes(f.key)
                ? 'bg-auricyn-elements-item-backgroundAccent border-auricyn-elements-borderColorActive shadow-lg'
                : 'bg-auricyn-elements-background-depth-3 border-auricyn-elements-borderColor hover:border-auricyn-elements-borderColorActive hover:bg-auricyn-elements-bg-depth-2'
            }`}
          >
            <div className="text-center space-y-2">
              <div
                className={`text-2xl font-medium transition-colors ${
                  font.includes(f.key) ? 'text-auricyn-elements-item-contentAccent' : 'text-auricyn-elements-textPrimary'
                }`}
              ...
              <div
                className={`text-sm font-medium transition-colors ${
                  font.includes(f.key) ? 'text-auricyn-elements-item-contentAccent' : 'text-auricyn-elements-textSecondary'
                }`}
              >
                {f.label}
              </div>
              {font.includes(f.key) && (
                <div className="w-6 h-6 mx-auto bg-auricyn-elements-item-contentAccent rounded-full flex items-center justify-center">
          ...
          return (
            <div key={f.key} className="feature-card-container p-2">
              <button
                ...
                className={`group relative w-full p-6 text-sm font-medium transition-all duration-200 bg-auricyn-elements-background-depth-3 text-auricyn-elements-item-textSecondary ${
                  f.key === 'rounded'
                    ? isSelected
                      ? 'rounded-3xl'
                      : 'rounded-xl'
                  ...
                  f.key === 'border'
                    ? isSelected
                      ? 'border-3 border-auricyn-elements-borderColorActive bg-auricyn-elements-item-backgroundAccent text-auricyn-elements-item-contentAccent'
                      : 'border-2 border-auricyn-elements-borderColor hover:border-auricyn-elements-borderColorActive text-auricyn-elements-textSecondary'
                    : f.key === 'gradient'
                      ? ''
                      : isSelected
                        ? 'bg-auricyn-elements-item-backgroundAccent text-auricyn-elements-item-contentAccent shadow-lg'
                        : 'bg-auricyn-elements-bg-depth-3 hover:bg-auricyn-elements-bg-depth-2 text-auricyn-elements-textSecondary hover:text-auricyn-elements-textPrimary'
                } ${f.key === 'shadow' ? (isSelected ? 'shadow-xl' : 'shadow-lg') : 'shadow-md'}`}
                style={{
                  ...(f.key === 'gradient' && {
                    background: isSelected
                      ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'
                      : 'var(--auricyn-elements-bg-depth-3)',
                    color: isSelected ? 'white' : 'var(--auricyn-elements-textSecondary)',
                  }),
                }}
              >
                <div className="flex flex-col items-center gap-4">
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-auricyn-elements-bg-depth-1 bg-opacity-20">
                    {f.key === 'rounded' && (
                      <div
    ...
    <div>
      ...
      <DialogRoot open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <Dialog>
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            <div className="">
              <DialogTitle className="text-2xl font-bold text-auricyn-elements-textPrimary">
                Design Palette & Features
              </DialogTitle>
              <DialogDescription className="text-auricyn-elements-textSecondary leading-relaxed">
            ...
            {/* Navigation Tabs */}
            <div className="flex gap-1 p-1 bg-auricyn-elements-bg-depth-3 rounded-xl">
              {[
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-all duration-200 ${
                    activeSection === tab.key
                      ? 'bg-auricyn-elements-background-depth-3 text-auricyn-elements-textPrimary shadow-md'
                      : 'bg-auricyn-elements-background-depth-2 text-auricyn-elements-textSecondary hover:text-auricyn-elements-textPrimary hover:bg-auricyn-elements-bg-depth-2'
                  }`}
                >
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-3">
                ...
                <Button
                  variant="ghost"
                  onClick={handleSave}
                  className="bg-auricyn-elements-button-primary-background hover:bg-auricyn-elements-button-primary-backgroundHover text-auricyn-elements-button-primary-text"
                >
                  Save Changes
                </Button>
              </div>
      ...
      <style>{`
        .custom-scrollbar {
          ...
          scrollbar-color: var(--auricyn-elements-textTertiary) transparent;
        }
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background-color: var(--auricyn-elements-textTertiary);
          border-radius: 3px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background-color: var(--auricyn-elements-textSecondary);
        }
        .line-clamp-2 {
          display: -webkit-box;
          -webkit-line-clamp: 2;
